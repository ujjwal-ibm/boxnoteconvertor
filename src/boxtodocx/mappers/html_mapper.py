...
class HTMLMapper:
    ...
    def _map_image(self, element: BoxElement) -> str:
        ...
        original_src = element.attrs.get('src', '')
        if not original_src:
            logger.warning("Image element found without src attribute")
            return ''
        
        # Get the mapped local path for this image
        if mapped_path := self.image_paths.get(original_src):
            # Ensure path uses forward slashes for HTML
            html_path = str(mapped_path).replace('\\', '/')
            alt_text = html.escape(element.attrs.get('alt', ''))
            logger.debug(f"Mapping image {original_src} to {html_path}")
            return f'<img src="{html_path}" alt="{alt_text}" />'
        else:
            logger.warning(f"No local file mapping found for image: {original_src}")
            return ''
